...
public class ComprovanteNFCE {
    ...
    public static String RegistrarItem_Tela_Itens(String Item ,String cDescricao, String cCodigo, Double nPreco, Double nQt, Float nDesc, Double nSubTotal){
            return RegistrarItem_Tela_Itens(Item, cDescricao, cCodigo, nPreco, nQt, nDesc, nSubTotal,false,false);
    }
   
    public static String RegistrarItem_Tela_Itens(String Item ,String cDescricao, String cCodigo, Double nPreco, Double nQt, Float nDesc, Double nSubTotal,boolean bRemoverCRLF, boolean  bFracionado )
    {
    ...
    public static String RegistrarItem_Tela_CabecalhoNota_DadosEmpresa(ResultSet rsDadosEmpresa )
    {
        ...
        try {
            ...
            String cBairro =  TratamentoNulos.getTratarString().Tratar( rsDadosEmpresa.getString("bairro"),"") ; //Form1.LojaAtual.Recordset!estado), " ", Form1.LojaAtual.Recordset!estado)
            
            String cCEP  =  TratamentoNulos.getTratarString().Tratar( rsDadosEmpresa.getString("cep"),"");
            
            if(!Fantasia.equalsIgnoreCase("")){
                cDados = Fantasia + ManipulacaoArquivo2.newline;
            }
            if(!cNomeEmpresaAtiva.equalsIgnoreCase("")){
               cDados += cNomeEmpresaAtiva + ManipulacaoArquivo2.newline;
            }            
            if(cEndeEmpresaAtiva.trim().length()+cFoneEmpresaAtiva.trim().length()>0){               
               cDados += cEndeEmpresaAtiva + " " + cNumero + (!cFoneEmpresaAtiva.equalsIgnoreCase("") ? " - Fone: " + cFoneEmpresaAtiva : "") + ManipulacaoArquivo2.newline;
            }
            if(cBairro.trim().length()+ cCEP.trim().length()+cCidadeEmpresaAtiva.trim().length()+ cEstadoEmpresaAtiva.trim().length()>0){
                cDados += cBairro + " " + cCEP + " "+ cCidadeEmpresaAtiva + "-" + cEstadoEmpresaAtiva + ManipulacaoArquivo2.newline;
            }            
    ...
    public static String DadosCliente_Conteudo(Dadosorc d){
        ...
        try {
            ...
            {
                cDados+=ComprovanteNFCE.RegistrarItem_Tela_CabecalhoNota_DadosCliente(Long.parseLong(d.getCodcliente()));
            }
        } catch (Exception e) {
            LogDinnamus.Log(e);
        }
    ...
    public static String Troca_CabecaNota_Conteudo(ResultSet rsDadosEmpresa, boolean bIgnorarCRLF,boolean ImprimirDadosLojas, Long CodigoTroca)
    {
        ...
        try {
            ...
            cDados+=ComprovanteNFCE.RegistrarItem_Tela_LinhaSeparadora() + ManipulacaoArquivo2.newline ;

            cDados+=ComprovanteNFCE.Troca_CabecalhoNota(CodigoTroca);

            //cDados+=ComprovanteNaoFiscal.RegistrarItem_Tela_LinhaSeparadora() + ManipulacaoArquivo2.newline;

            if(ImprimirDadosLojas){
                cDados += ComprovanteNFCE.RegistrarItem_Tela_CabecalhoNota_DadosEmpresa(rsDadosEmpresa) + ManipulacaoArquivo2.newline;
            }                    
            cDados+=ComprovanteNFCE.Troca_CabecalhoNota_DescritivoDosItens(bIgnorarCRLF);
    ...
    public static String CabecaNota_Conteudo(Dadosorc d, ResultSet rsDadosEmpresa, String cTipoNota, boolean bIgnorarCRLF , ArrayList<Long> Notas )
    {
        ...
        String cDados="";
        try {            
            //cDados+=ComprovanteNFCE.RegistrarItem_Tela_CabecalhoNota_TipoNota(cTipoNota);           
            cDados+="NFCe - Nota Fiscal de Consumidor Eletronica" + ManipulacaoArquivo2.newline; 
            //if(rsDadosEmpresa.getBoolean("imprimirdadosdalojanota"))            {
                cDados += ComprovanteNFCE.RegistrarItem_Tela_CabecalhoNota_DadosEmpresa(rsDadosEmpresa) + ManipulacaoArquivo2.newline;
            //}
            cDados+= CabecaNota_Conteudo_DadosPDV(d,cTipoNota,true,Notas);           
            cDados+= ComprovanteNFCE.RegistrarItem_Tela_LinhaSeparadora() +  ManipulacaoArquivo2.newline;
            cDados+="DANFE NFCe - Documento Auxiliar de Nota Fiscal" +  ManipulacaoArquivo2.newline;
            cDados+="de Consumidor Eletronica" +  ManipulacaoArquivo2.newline;
            cDados+="NAO PERMITE APROVEITAMENTO DE CREDITO DE ICMS" +  ManipulacaoArquivo2.newline;
            cDados+=ComprovanteNFCE.RegistrarItem_Tela_CabecalhoNota_DescritivoDosItens(bIgnorarCRLF);
        } catch (Exception e) {
        ...
        try {
            ...
            if(ExibirDadosCompl || ForcaExibicao){                
                    ...
                    for (int i = 0; i < PreVendas.size(); i++) {
                        ResultSet rsDadosVenda = PreVenda.ListarPreVendas(PreVendas.get(i), Sistema.getLojaAtual());
                         ...
                         if(rsDadosVenda.next()){
                            ...
                            if(PreVendas.size()>1){
                                if(i==0){
                                    LinhasPreVenda+=ComprovanteNFCE.RegistrarItem_Tela_LinhaSeparadora() +  ManipulacaoArquivo2.newline;
                ...
                if(d.getCodvendedor().indexOf(UsuarioSistema.getCodVendedorVinculadoAoCaixa()) <0){                                      
                    cDados+="VEND : " + d.getCodvendedor() + " - " + d.getVendedor() +ManipulacaoArquivo2.newline;                                        
                }
                if(ExibirDataHora){
    ...
    public static String ComprovanteCrediario_Contas_Conteudo(Dadosorc d , BaixarConta b)
    {
        ...
        try {
            ...
            int TamLinha = ComprovanteNFCE.RegistrarItem_Tela_LinhaSeparadora().length();
            boolean ExibirDadosCompl = Sistema.getDadosLoja(Sistema.getLojaAtual(), true).getBoolean("ExibirDadosComplCF");
     ...
     public static String RegistrarItem_Imprimir_Itens(String Item ,String cDescricao, String cCodigo, Double nPreco, Double nQt, Float nDesc, Double nSubTotal,boolean bRemoverCRLF, boolean  bFracionado,String Cor,String Tam )
    {
       ...
       //         12345678901234567890123456789012345678901234567890
        ...
        cDados   =  "("+ManipulacaoString.Replicate("0", 3 - Item.trim().length())+ Item +") - "+   
                cDescricao.substring(0, (cDescricao.trim().length()>=40 ? 40 : cDescricao.trim().length())) ;
             
        if(cDados.length()<40){
            cDados =ManipulacaoString.PrepararStringEmLinha_Direita(cDados, 40, ".");                            
        }
        cDados =ManipulacaoString.Left(cDados, 40);        

        String DadosCompl=(Cor + " " + Tam).trim();
        if(cDados.length()<48){
    ...
    public static String DadosFechamentoNFCe(Long CodigoVenda){
        String Ret = "";
        try {
            ResultSet rs= DAO_RepositorioLocal.GerarResultSet("select * from off_nfce_pdv_notas where codigovenda=" + CodigoVenda);
            if( rs!=null){
                if(rs.next()){
                    String Numero="",URLPesquisa="",Serie="",ChaveDeAcesso="",Protocolo="";
                    int TipoEmissao = TratamentoNulos.getTratarInt().Tratar(rs.getInt("tipoemissao"),0);
                    Numero =  TratamentoNulos.getTratarString().Tratar(rs.getString("numero"),"");
                    Serie =  TratamentoNulos.getTratarString().Tratar(rs.getString("serie"),"");
                    ChaveDeAcesso =  TratamentoNulos.getTratarString().Tratar(rs.getString("chave"),"");
                    Protocolo =    TratamentoNulos.getTratarString().Tratar(rs.getString("numeroProtocolo"),"");
                    /*if(Protocolo.length()>0){
                        List<NFProtocolo> protocolos =( List<NFProtocolo>) XStream_Api.ConverterXmlParaObjeto(Protocolo);
                        if(protocolos.size()>0){
                           Protocolo = protocolos.get(0).getProtocoloInfo().getNumeroProtocolo();
                        }
                    }*/
                    Integer Ambiente =TratamentoNulos.getTratarInt().Tratar(rs.getInt("ambiente"),0);
                    Timestamp DataHoraAutorizacao=rs.getTimestamp("datahoraautorizacao");
                    Timestamp Emissao=rs.getTimestamp("emissao");
                    
                    //Ret+= RegistrarItem_Tela_LinhaSeparadora() + ManipulacaoArquivo2.newline ;
                    
                    //Ret+="Emitida em : " + DataHora.getData("dd/MM/yyyy HH:mm:ss", Emissao) + ManipulacaoArquivo2.newline; 
                    if(Protocolo.length()>0){
                        Ret+="Procotolo  : " + Protocolo + ManipulacaoArquivo2.newline;
                        Ret+="Autorizada :" + DataHora.getData("dd/MM/yyyy HH:mm:ss", DataHoraAutorizacao)+ ManipulacaoArquivo2.newline;
                    }
                    if(TipoEmissao==9){
                        Ret+="===========================" + ManipulacaoArquivo2.newline;
                        Ret+="NFCe EMITIDA EM CONTIGENCIA" + ManipulacaoArquivo2.newline;
                        Ret+="===========================" + ManipulacaoArquivo2.newline;
                    }
                    Ret+="Numero     : "+   Numero + " - Serie : " + Serie + ManipulacaoArquivo2.newline ;                    
                    if(NFAmbiente.HOMOLOGACAO.getCodigo().equalsIgnoreCase(Ambiente.toString())){
                        URLPesquisa="https://appnfc.sefa.pa.gov.br/portal-homologacao/view/consultas/nfce/consultanfce.seam";
                    }else{
                        URLPesquisa="https://appnfc.sefa.pa.gov.br/portal/view/consultas/nfce/consultanfce.seam";
                    }
                    Ret+=ManipulacaoArquivo2.newline;
                    Ret+="Consulta pela Chave de Acesso em : "+ URLPesquisa + ManipulacaoArquivo2.newline ;      
                    Ret+=ManipulacaoArquivo2.newline;
                    Ret+="Chave de Acesso "  + ManipulacaoArquivo2.newline;
                    Ret+=ChaveDeAcesso + ManipulacaoArquivo2.newline;
    ...
    public static String TerminaFechamento(Float nDinheiro, Float nTroco, String cMensagem, Long nCodigoCliente, Long nCodigoVenda,String idCPF_CNPJ, String CodigoVendedor,String NomeVendedor){
        String cDados="";
        try {
            ...
            if(nCodigoCliente>0){
                ...
                if(!TratamentoNulos.getTratarString().Tratar( idCPF_CNPJ,"").equalsIgnoreCase("")){
                    cDados+= RegistrarItem_Tela_LinhaSeparadora() + ManipulacaoArquivo2.newline ; 
                    cDados+="IDENTIFICACAO CLIENTE : " + idCPF_CNPJ + ManipulacaoArquivo2.newline ;
                }
            }
    ...
    public static String DesdobramentoParcelas(Long nCodigoVenda){
        ...
        try {
            ...
            if(rsDadosCliente.next()){
                ...
                cRet =cRet  + ComprovanteNFCE.RegistrarItem_Tela_LinhaSeparadora() + ManipulacaoArquivo2.newline ;
            ...
            public static String GerarRelatorio(String cControleCX, int nCodigoLoja )
    {
        ...
        try {
            ...
            String cTexto = "";
            cTexto=cTexto+(ComprovanteNFCE.RegistrarItem_Tela_LinhaSeparadora()) + ManipulacaoArquivo2.newline ;
